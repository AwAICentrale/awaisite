...
from src.AIs.fantome import Fantome
...
class AI(Player):
    ...
    def __init__(self, algo, game, data=None):
        ...
        if self.algo == "alea":
            ...
            if data is None:
                data = [0.676061829383705, -0.4604896125458653, 0.7408590076155085, 0.3691310747575154]
            self.algo = AlphaBeta(self.game, 10, data)  # data is listecoeffgain
        ...
        elif self.algo == "fantome0":
            if data is None:
                data = [0,0,0]
            self.algo=Fantome(self.game,0, data)
        elif self.algo == "fantome2":
            if data is None:
                data = [0,0,0]
            self.algo=Fantome(self.game,2, data)
        elif self.algo == "fantome4":
            if data is None:
                data = [0,0,0]
            self.algo=Fantome(self.game,4, data)
        elif self.algo == "fantome6":
            if data is None:
                data = [0,0,0]
            self.algo=Fantome(self.game,6, data)
        elif self.algo == "fantome8":
            if data is None:
                data = [0,0,0]
            self.algo=Fantome(self.game,8, data)
